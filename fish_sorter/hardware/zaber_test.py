...
def main() -> bool:
    ...
    cfg_dir = Path(__file__).absolute().parent.parent / "configs/hardware"
    ...
    try:
        ...
        pick_cfg_dir = Path(__file__).absolute().parent.parent / "configs/hardware/"
        pick_cfg_file = "picker_config.json"
        pick_cfg_path = pick_cfg_dir / pick_cfg_file
        with open(pick_cfg_path, 'r') as f:
            p = load(f)
            pick_config = p['defaults']

        print("pick file started")
